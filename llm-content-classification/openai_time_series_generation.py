...
def generate_time_series_supply(community):
    # first load all the topic that we generated
    with open(f"{community}/openai/new/summary.txt", 'r') as file:
        topics = [topic.strip() for topic in file.read().splitlines()]
    
    with open(f"{community}/openai/new/{community}_word_og.json") as file:
        tweets_mapping = json.load(file)
    
    # get all the user information and the tweets that they posted 
    db = MONGO_CLIENT[community]
    ...
    og_tweet_time_series = db["og_tweet_time_series_word"]
    ...
    # for 2 day
    # period = pd.Timedelta(days=2)
    # time_stamps = pd.date_range(start=min_date, end=max_date, freq=period)
        ...
        for _, tweet in user_tweets.iterrows():
            ...
            if tweet_id_str in tweets_mapping:
                ...
                if topic in user_time_series_df.columns:
                    # time_bin = time_stamps[time_stamps.searchsorted(tweet_date, side='right') - 1]
                    # user_time_series_df.at[time_bin, topic] += 1
...
def generate_time_series_demand_in(community):
    # first load all the topic that we generated
    with open(f"{community}/openai/new/summary.txt", 'r') as file:
        topics = [topic.strip() for topic in file.read().splitlines()]
    
    with open(f"{community}/openai/new/{community}_word_in.json") as file:
        tweets_mapping = json.load(file)
    ...
    retweet_tweet_time_series = db["retweet_in_time_series_word"]
...
def generate_time_series_demand_out(community):
    # first load all the topic that we generated
    with open(f"{community}/openai/new/summary.txt", 'r') as file:
        topics = [topic.strip() for topic in file.read().splitlines()]
    
    with open(f"{community}/openai/new/{community}_word_out.json") as file:
        tweets_mapping = json.load(file)
    ...
    retweet_out_tweet_time_series = db["retweet_out_time_series_word"]
    ...
    for user in user_df['userid'].unique():
        ...
        retweet_out_tweet_time_series.insert_one(user_document)


if __name__ == "__main__":
    communities = ["science_fiction_expanded", "game_development", "urban_planning", "neuroscience_expanded"]
    for community in communities:
        # remove_none_type(f"{community}/openai/new/og_tweets_classification_new.json")
        # remove_none_type(f"{community}/openai/new/retweet_in_tweets_classification_new.json")
        # remove_none_type(f"{community}/openai/new/retweet_out_tweets_classification_new.json")
        generate_time_series_demand_in(community)
        generate_time_series_demand_out(community)
        generate_time_series_supply(community)
